...
[![Tech Stack](https://img.shields.io/badge/Stack-ASP.NET%20Core%20%2B%20Vue%203-blue)](https://github.com/dja41419-design/MyFitnessCoach_Client)
[![Frontend Repo](https://img.shields.io/badge/Frontend-GitHub-green)](https://github.com/dja41419-design/MyFitnessCoach_Client)
...
| 購物車管理介面 | 登入後自動合併機制 |
| :---: | :---: |
| ![購物車介面](./專題畫面截圖/購物車.jpg) | ![合併邏輯](./專題畫面截圖/購物車自動合併.png) |

*   **全站單例狀態管理**：使用 Vue Composable 結合 Module-level Singleton，確保 Navbar、商品列表與購物車頁面的資料毫秒級同步。
*   **離線優先合併演算法**：訪客購物資料持久化於 LocalStorage，登入後自動觸發後端 `MergeAsync` 邏輯，將本地與資料庫內容進行去重相加。

### 2. 🎟️ 優惠券推薦引擎 (Coupon Engine)
實作「自動挑選最省錢組合」的智能體驗。
...
*   `Models/Services/CouponService.cs` - 核心優惠券試算與推薦邏輯
...
## 💡 開發體悟
在開發過程中，我體會到「狀態管理」是電商系統的靈魂。透過 Vue Composable，我成功將複雜的購物車與優惠券邏輯從 UI 元件中抽離，不僅提高了程式碼的測試性，也讓維護難度大幅降低。同時，在處理後端效能時，深入理解了資料庫延遲執行的重要性。
