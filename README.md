...
## 開發體悟
在專案開發過程中，我對前後端開發各有不同的體會。

前端方面，我體認到狀態管理在電商系統中的重要性。透過 Vue Composable 將購物車與優惠券邏輯從元件中抽離，不僅提升程式碼的可重用性，也讓功能維護與後續擴充更加容易。

後端方面，三層式架構讓我學習到如何將請求處理、商業邏輯與資料存取職責分離，使程式結構更加清晰。在優惠券功能中，我將折扣試算與資格驗證放在後端處理，避免前端直接決定折扣金額；而前端則負責推薦與排序邏輯，讓系統責任劃分更加明確。

此外，在商品搜尋功能開發過程中，我透過 LINQ 動態組合查詢條件，並利用 Deferred Execution 避免不必要的資料載入，讓我更理解資料查詢效能的重要性。
